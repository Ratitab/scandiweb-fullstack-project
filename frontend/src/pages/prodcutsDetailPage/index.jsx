...
class ProductDetailPage extends Component {
  ...
  isAddToCartDisabled = () => {
    const { PDPDetails, selectedAttributes } = this.state;
    return PDPDetails.attributes && PDPDetails.attributes.some(
      (attr) => !selectedAttributes[attr.name]
    );
  };

  render() {
    const { PDPDetails, selectedAttributes, currentImageIndex, cart, isCartOpen } = this.state;
          ...
          <div className="relative flex-1 flex justify-center items-center">
            <button
              onClick={() => this.setState((prevState) => ({
                currentImageIndex: (prevState.currentImageIndex - 1 + PDPDetails.image_gallery.length) % PDPDetails.image_gallery.length
              }))}
            ...
            <img
              src={PDPDetails.image_gallery ? PDPDetails.image_gallery[currentImageIndex] : ""}
              alt={PDPDetails.name}
            ...
            <button
              onClick={() => this.setState((prevState) => ({
                currentImageIndex: (prevState.currentImageIndex + 1) % PDPDetails.image_gallery.length
              }))}
        ...
        <div className="flex flex-col px-12 w-1/2">
          ...
          {PDPDetails.attributes &&
            PDPDetails.attributes.map((attribute) => (
              <div key={attribute.name} className="mt-6">
                <h4 className="font-semibold tracking-wide">{attribute.name.toUpperCase()}:</h4>
                <div className="flex gap-2 mt-2">
                  {attribute.items.map((item) => (
                    <button
                      key={item.value}
                      onClick={() => this.handleAttributeClick(attribute.name, item)}
                      ...
                      style={
                        attribute.name === "Color"
                          ? { backgroundColor: item.value, width: "24px", height: "24px" }
                          : {}
                      }
                    >
          ...
          <div className="mt-8">
            ...
            <p className="font-bold mt-4 text-xl">
              {PDPDetails.price?.currency_symbol}{PDPDetails.price?.amount}
            </p>
          </div>
